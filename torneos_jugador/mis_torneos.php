<?php
    ...
    $sql = "SELECT t.titulo, j.nombre AS juego, t.fecha, t.hora, m.nombre AS modalidad, f.nombre AS forma, t.max_jugadores, t.premios, t.descripcion, e.nombre AS estatus, ji.lugar
            FROM torneos t, juegos j, modalidades m, formas f, gamers g, estatus e, jugadores_inscritos ji
            WHERE t.id_juego = j.id AND t.id_modalidad = m.id AND t.id_forma = f.id AND t.id_estatus = e.id AND t.id = ji.id_torneo AND g.id = ji.id_jugador AND g.id = $id";
